...
Additionally, contrasting both trait and genetic differentiation across several populations, Q$_\text{ST}$--F$_\text{ST}$ methods have allowed to determine the selective regime and to quantify the strength of selection acting on a trait~\parencite{martin_multivariate_2008, leinonen_qst_2013}.
For example, studies have shown that stabilizing selection is largely dominant in the evolution of gene expression in different populations~\parencite{whitehead_neutral_2006, gilad_natural_2006}. [why only example from gene expression?]
However, dominant selective regime acting on gene expression is still controversial, and neutral evolution is still debated~\parencite{signor_evolution_2018, price_detecting_2022}.
...
At the frontier between micro and macroevolution, comparative methods at the phylogenetic scale have acknowledged the importance of modeling within-species variation on top of changes in mean trait value~\parencite{fay_evaluating_2008, kostikova_bridging_2016, gaboriau_multiplatform_2020, gaboriau_exploring_2023}.
However, in the context of detecting selection acting on a trait, within-species variation is mostly used to describe technical errors.
One notable exception used the ratio of between to within variation estimated for many traits, and compared to the average over all traits to seek deviation from this average~\parencite{rohlfs_modeling_2014, rohlfs_phylogenetic_2015}.
...
For population geneticists, our study can be seen as the macro-evolutionary generalization of Q$_\text{ST}$--F$_\text{ST}$ methods to account for phylogenetic relationships between species.
...
The additive genetic variance is also equal to the observed phenotypic variance ($\VarPhenotype$) multiplied by narrow-sense heritability ($\Heritability$; \parencite{hill_data_2008}), which leads to $\RateWhithin$ beinf a function of $\VarPhenotype$ and $\Heritability$:
\begin{align}
...
[NS: the start of this paragraph is not so clear -> to discuss]
Even though the neutrality index can be applied to any trait, we assume here that the variation between several traits is independent.
Assuming covariation between traits is also an extension that can be implemented within the multidimensional Brownian framework~\parencite{huelsenbeck_detecting_2003, lartillot_phylogenetic_2011, lartillot_joint_2012, latrille_inferring_2021}.
Here we generalize to $\Ntrait$ non-independent traits evolving along the phylogeny using the \textit{BayesCode} software~\parencite{latrille_inferring_2021}.
...
We tested the xxx [NS: explain a bit what we test here -> NI]
We simulated phenotypic values as the sum of genotypic value and an environmental effect.
...
We followed a Wright-Fisher model with mutation, selection and drift to formalize the evolutionary dynamics of the system (Fig.~\ref{fig:simulator}A\&B).
...
Additionally, the instant value of log-$\Ne$ was modeled as a sum of a geometric Brownian process and an Ornstein-Uhlenbeck process ($\text{OU} \left(0, \sigma_{\Ne}=0.1, \theta_{\Ne}=0.9\right)$).
The geometric Brownian motion accounted for long-term fluctuations, while the Ornstein-Uhlenbeck introduced short-term fluctuations. [NS: not clear how you combined both here]
...
At the species level, the additive genetic variance of the neutral trait ($\VarGenetic$) is normalized by nucleotide diversity of any neutral genomic region ($\pi$) to obtain $\RateWhithin$.
At the scale of between-species variation, assuming a neutral trait evolving along the phylogeny, the mean trait value changes as a Brownian process.
This variance in mean trait value can also be normalized by the nucleotide divergence of any neutral genomic region ($d$) to obtain $\RateBetween$. [NS: -> this is repeating the mat/met, is it really needed??]
...
[NS: this is not a result but an assumption that we have. We also already say it above. I would move it to discussion if needed]
...
The relationship between trait variation between and within species can be used as a method to detect whether traits are evolving under a neutral model of evolution.
...
\subsection*{Test of selection}

[NS: here again, these are not results, but a discussion]
...
With the aim of applying the inference method to empirical datasets, the simulation parameters were chosen so as to match an empirically relevant mammalian empirical regime (see Materials and Methods). [NS: I would remove this sentence]
...
If we assumed that heritability ($\Heritability$) of body mass was uniformly distributed between 20\% and 40\%, posterior probabilities of diversifying selection became $0.635$ for males and $0.324$ for females.
...
Fixing the heritabity of body mass to $1.0$ did not change the posterior probability for males, but increased the one for female to $1.0$.
...
In other words, our statistical test seeks to find traits with extremal values in comparison to a pool of traits.
Instead, our procedure can be applied to a single trait [NS: this is not clear as the previous sentence seems to contradict this one], estimating the neutrality index and giving a statistical test for departure from the null model of neutral evolution.
...
By doing so, we also show using simulated data that our test is not sensitive to the assumption that population sizes ($\Ne$) and mutation rates ($\MutationRate$) are constant across the phylogeny, while not requiring to estimate them nor to estimate divergence time~\parencite{litsios_effects_2012}.
Instead, the normalization by nucleotide divergence and polymorphism automatically absorbs these changes. [NS: we need to develop these arguments and show that the normalization works because it is similar to the dN/dS tests, etc.... I would thus move the paragraph below on this here. Otherwise, this paragraph is a bit hollow]

% Pitfalls of the method
In the context of detecting diversifying selection, we argue that the main drawback of our method is that the narrow-sense heritability ($\Heritability$) of a trait is required if one has access only to the phenotypic variance and not the additive genetic variance. [NS: a bit circular. If we have V_G, we know h^2 as V_P can be measured]
...
Because our test is also based on several assumptions that might not hold on empirical data, we also provide a table containing the main assumptions and their consequences on the neutrality index and the test that can be performed (see table~\ref{table:assumptions}). [NS: I would discuss the elements of this table and link that with some literature if possible.]
As an example, even though we assumed complete heritability for brain mass, we can assert that brain mass is evolving under diversifying selection in mammals since $\EstNI > 1$.
...
Second, loci are assumed to contribute additively to the phenotype~\parencite{hill_data_2008, crow_epistasis_2010}, disregarding effects of dominance and epistasis. [NS: here a few words on the importance of dominance and epistasis in empirical quant genet studies would be interesting. And what about long-term effects at the phylogenetic level?]
Third, the genetic architecture of the trait is assumed to be constant across the phylogeny, which might actually be variable among individuals and species. [NS: any literature on this (or is it the following ones)? I'm not aware of that right now, but we should have a look]
...
[NS: should we not start with this idea?]
...
At the phylogenetic scale, tests for neutrality of a quantitative trait are seeking deviation from a Brownian process~\parencite{catalan_drift_2019}, or from an Ornstein–Uhlenbeck process accounting for within-species variation~\parencite{rohlfs_phylogenetic_2015}.
The analysis on the transcriptome across different individuals and species concluded that the expression level of 81\% of genes are under a neutral regime and 16\% under directional selection~\parencite{catalan_drift_2019}.
...
Our diversity index can thus allow us to revisit these studies, assuming we have access to genomic datasets to estimate nucleotide divergence ($d$) and polymorphism ($\pi$).
Even though our method requires integrating genomic and trait variation, we also showed that our method can be applied to the empirical dataset with the illustrative example of mammals' brain and body mass.
