...
%DIF DEL main-first-submission.tex   Wed Mar  6 16:12:09 2024
%DIF ADD main-revision.tex           Wed Mar 13 15:41:30 2024
...
%DIF 87a86-88
\newcommand{\Indiv}{k} %DIF > 
\newcommand{\Branch}{b} %DIF > 
\newcommand{\WishartIDD}{m} %DIF > 
%DIF -------
\newcommand{\Spi}{i}
\newcommand{\Spj}{j}
\newcommand{\NbrTaxa}{n}
...
%DIF -------
\newcommand{\EstRateBetween}{\widehat{\sigma}^2_{\mathrm{B}}} %DIF > 
\newcommand{\EstRateWhithin}{\widehat{\sigma}^2_{\mathrm{W}}} %DIF > 
%DIF -------
\newcommand{\NI}{\rho}
...
\DIFaddbegin \DIFadd{Given this genetic architecture of traits, from two diverging populations, it is possible to distinguish which traits have evolved under natural selection in controlled experimental settings, by performing genetic cross between individuals~\mbox{%DIFAUXCMD
\citep{fraser_detecting_2020}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend Across several populations, by contrasting both trait \DIFaddbegin \DIFadd{differentiation (\Qst) }\DIFaddend and genetic differentiation \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(\Fst), so-called }\DIFaddend \QstFst\ methods have been used to determine the selective regime and to quantify the strength of selection acting on a trait~\citep{merila_comparison_2001, leinonen_comparative_2008}.
\DIFdelbegin \DIFdel{A trait differentiation (\Qst) higher than genetic differentiation (\Fst) }\DIFdelend \DIFaddbegin \DIFadd{\Qst\ higher than \Fst\ }\DIFaddend is interpreted as a signature of diversifying selection due to adaptation \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend different optimum trait \DIFdelbegin \DIFdel{value }\DIFdelend \DIFaddbegin \DIFadd{values }\DIFaddend in the different populations\DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{lamy_qst_2012}}\hskip0pt%DIFAUXCMD
}\DIFdelend .
Contrarily, \Qst\ lower than \Fst\ is interpreted as a signature of stabilizing selection\DIFdelbegin \DIFdel{.
However, \QstFst\ methods have been found to require many populations~\mbox{%DIFAUXCMD
\citep{ohara_bias_2005}}\hskip0pt%DIFAUXCMD
, and that various factors can generate a spurious signal of selection~\mbox{%DIFAUXCMD
\citep{pujol_are_2008, edelaar_comparisons_2011}}\hskip0pt%DIFAUXCMD
.
Moreover, the test for diversifying selection is }\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep{lamy_qst_2012}}\hskip0pt%DIFAUXCMD
.
...
This integrated framework can discriminate between selection and genetic drift as a cause of trait differentiation between populations of the same species~\mbox{%DIFAUXCMD
\citep{ovaskainen_new_2011}}\hskip0pt%DIFAUXCMD
.
However, regardless of the strengths and weaknesses of each method\mbox{%DIFAUXCMD
\citep{pujol_are_2008, edelaar_comparisons_2011, ovaskainen_new_2011}}\hskip0pt%DIFAUXCMD
, tests of trait differentiation between populations are ultimately }\DIFaddend limited to recent local adaptation since \DIFdelbegin \DIFdel{the test is }\DIFdelend \DIFaddbegin \DIFadd{they are }\DIFaddend based on the variation observed within a single species.
\DIFaddbegin 

%DIF >  Pair of species
\DIFaddend To disentangle selection from neutral evolution, trait variation can also be observed at a larger time scale.
For example, \DIFdelbegin \DIFdel{change in mean trait value accumulates }\DIFdelend \DIFaddbegin \DIFadd{starting from the same ancestral population, divergent lineages accumulate phenotypic changes that will reach fixation in the population.
...
}\DIFdelend \DIFaddbegin \DIFadd{As an analogy, in the context of protein-coding DNA sequences, leveraging within species variation and divergence to a sister species is the crux of the \mbox{%DIFAUXCMD
...
From the field of population genetics, \DIFaddbegin \DIFadd{while \QstFst\ methods and their derivatives ultimately seek trait differentiation among different populations from the same species~\mbox{%DIFAUXCMD
\citep{pujol_are_2008, ovaskainen_new_2011}}\hskip0pt%DIFAUXCMD
, }\DIFaddend our study can be seen as \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{their }\DIFaddend macro-evolutionary generalization \DIFdelbegin \DIFdel{of \QstFst\ methods }\DIFdelend to account for phylogenetic relationships between species.
...
%DIF <  Basically, we are testing whether the variance of means is greater than the mean of variances
...
As shown in \citet{lande_quantitative_1979, lande_sexual_1980}, $\VarMutation$ is a function of the mutation rate per \DIFdelbegin \DIFdel{loci }\DIFdelend \DIFaddbegin \DIFadd{locus }\DIFaddend per generation ($\MutationRatePheno$) and \DIFdelbegin \DIFdel{$\RateMut$:
...
As shown in \citet{tajima_statistical_1989}, $\pi$ is a function of the mutation rate \DIFdelbegin \DIFdel{per loci per generation}\DIFdelend (\DIFdelbegin \DIFdel{$\MutationRatePheno$}\DIFdelend \DIFaddbegin \DIFadd{$\MutationRateNuc$, per nucleotide site per generation}\DIFaddend ) and the effective population size ($\Ne$):
\begin{gather}
...
\DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{To remove the effect of $\Ne$, we }\DIFaddend define $\RateWhithin$ as the ratio of additive genetic variance of the trait ($\VarGenetic$) over $\pi$ of any neutral genomic region of interest.
\DIFdelbegin \DIFdel{This ratio allows removing the effect of $\Ne$ and $\MutationRatePheno$, which are parameters not related to the genetic architecture of the trait, giving $\RateWhithin$ asa proxy of $\RateMut$:
}\DIFdelend \DIFaddbegin \DIFadd{After simplification, $\RateWhithin$ is then solely a function of the underlying genetic architecture as:
...
\DIFdelbegin \DIFdel{The additive genetic variance is also equal }\DIFdelend \DIFaddbegin \DIFadd{If $\VarGenetic$ is not empirically accessible, it can be related }\DIFaddend to the observed phenotypic variance ($\VarPhenotype$)\DIFaddbegin \DIFadd{, }\DIFaddend multiplied by narrow-sense heritability \DIFaddbegin \DIFadd{of the trait }\DIFaddend ($\Heritability$\DIFdelbegin \DIFdel{; \mbox{%DIFAUXCMD
\citep{hill_data_2008}}\hskip0pt%DIFAUXCMD
), which }\DIFdelend \DIFaddbegin \DIFadd{), as~\mbox{%DIFAUXCMD
\citep{hill_data_2008}}\hskip0pt%DIFAUXCMD
:
}\begin{align}
...
}\DIFdelend \DIFaddbegin \DIFadd{instead of $\VarGenetic$ as:
}\DIFaddend \begin{align}
...
by:
    ...
    & = 4 \DIFdelbegin \DIFdel{\Time }\DIFdelend \DIFaddbegin \DIFadd{\NbrGen }\DIFaddend \Multiply \MutationRatePheno \Multiply \DIFdelbegin \DIFdel{\RateMut }\DIFdelend \DIFaddbegin \GenArchi \DIFaddend \label{eq:covar},\text{ from eq.~\ref{eq:var-genetic}}\DIFdelbegin \DIFdel{,
}\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend \end{align}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Moreover, for any genomic region under neutral evolution, some mutations will eventually reach fixation due to random genetic drift, resulting in a substitution of a nucleotide at the species level.
...
We can derive the substitution rate \DIFdelbegin \DIFdel{per generation }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend $\SubRate$\DIFaddbegin \DIFadd{, per nucleotide site per generation) }\DIFaddend as the number of \DIFdelbegin \DIFdel{mutations per generation ($2\Ne \Multiply \MutationRatePheno$}\DIFdelend \DIFaddbegin \DIFadd{newly arisen mutations ($2\Ne \Multiply \MutationRateNuc$}\DIFaddend ) multiplied by the probability of fixation for each newly arisen mutations $\pfix$~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
\end{align}
That is, if mutations are neutral, the rate of substitution \DIFaddbegin \DIFadd{per generation }\DIFaddend within a genomic region equals the rate at which new mutations arise per generation for the same genomic region~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{kimura_evolutionary_1968}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
...
Assuming }\DIFaddend that no multiple substitutions occurred at the same site, \DIFdelbegin \DIFdel{the nucleotide divergence $d$, which is the fraction of the genomic region that generated a substitution, will be $\Time$ }\DIFdelend \DIFaddbegin \DIFadd{$\NucDiv$ is the number of generations ($\NbrGen$) }\DIFaddend multiplied by the nucleotide substitution rate per generation ($\SubRate$):
\begin{align}
...
\end{align}
\DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{To remove the effect of the number of generations ($\NbrGen$) first, and to also equate to $\RateWhithin$ (eq.~\ref{eq:rate-pop}), we }\DIFaddend define $\RateBetween$ as the \DIFdelbegin \DIFdel{variance }\DIFdelend \DIFaddbegin \DIFadd{covariance }\DIFaddend in the mean trait value ($\VarPhy$) normalized by \DIFaddbegin \DIFadd{4 times }\DIFaddend the nucleotide divergence of any neutral genomic region (\DIFdelbegin \DIFdel{$d$).
This ratio allows removing the effect of $\Time$ and $\MutationRatePheno$, which are parameters not related to }\DIFdelend \DIFaddbegin \DIFadd{$4\NucDiv$).
After simplification, $\RateBetween$ is also solely a function of the underlying genetic architecture as:
}\begin{align}
    \DIFadd{\RateBetween }& \DIFadd{\defEqual \frac{\VarPhy}{4 \NucDiv} \label{eq:def-rate-phy},}\\
    & \DIFadd{= \frac{4 \NbrGen \Multiply \MutationRatePheno \Multiply \GenArchi}{4 \NbrGen \Multiply \MutationRateNuc}\text{ from eq.~\ref{eq:covar} and~\ref{eq:distance}}, }\\
    & \DIFadd{= \frac{\MutationRatePheno \Multiply \GenArchi}{\MutationRateNuc}. \label{eq:rate-phy}
}\end{align}

\DIFadd{In equation~\ref{eq:rate-phy}, we show that the covariance in mean trait value between a pair of species ($\VarPhy$) does increase linearly with shared nucleotide divergence ($\NucDiv$), if the trait and sequences are neutrally evolving and }\DIFaddend the genetic architecture of the \DIFdelbegin \DIFdel{trade, giving $\RateBetween$ as another proxy of $\RateMut$:
}\begin{align*}
    \DIFdel{\RateBetween }& \DIFdel{\defEqual \frac{\VarPhy}{4 d},}\\
    & \DIFdel{= \frac{4 \Time \Multiply \MutationRatePheno \Multiply \RateMut}{4 \Time \Multiply \MutationRatePheno}\text{ from eq.~\ref{eq:covar} and~\ref{eq:distance}}, }\\
    & \DIFdel{= \RateMut %DIFDELCMD < \label{eq:rate-phy}%%%
.
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{trait has not changed.
...
Both $\RateBetween$ and $\RateWhithin$ can be estimated using quantitative trait and genomic sequences within and between species, while neither the mutation \DIFdelbegin \DIFdel{rate }\DIFdelend \DIFaddbegin \DIFadd{rates }\DIFaddend ($\MutationRatePheno$ \DIFaddbegin \DIFadd{and $\MutationRateNuc$}\DIFaddend ), nor the effective population size ($\Ne$)\DIFaddbegin \DIFadd{, generation time }\DIFaddend or time of divergence (\DIFdelbegin \DIFdel{$\Time$}\DIFdelend \DIFaddbegin \DIFadd{$\NbrGen$}\DIFaddend ) need to be estimated.
...
The off-diagonal elements (\DIFdelbegin \DIFdel{$\Distance_{\Spi,\Spj} = \Distance_{\Spj,\Spi}$}\DIFdelend \DIFaddbegin \DIFadd{$\Distance_{\Spi,\Spj} = \NucDiv$}\DIFaddend ) are the distances between the root and the most recent common ancestor of taxa $\Spi$ and $\Spj$\DIFdelbegin \DIFdel{.
The state $\RootTrait$ }\DIFdelend \DIFaddbegin \DIFadd{, as in equation~\ref{eq:distance}.
The mean trait value }\DIFaddend at the root of the tree \DIFdelbegin \DIFdel{for the trait }\DIFdelend \DIFaddbegin \DIFadd{($\RootTrait$) }\DIFaddend can be estimated from the $\NbrTaxa \times 1$ vector of mean trait values $\VecTrait$ at the tips of the tree using maximum likelihood~\citep{omeara_testing_2006}:
\begin{gather}
...
The \DIFaddbegin \DIFadd{branch lengths of the tree used to model the Brownian process runs is measured in units of $4d$ ($d$ being the nucleotide divergence).
...
\DIFdelbegin \DIFdel{Similarly, }\DIFdelend \DIFaddbegin \DIFadd{Additionally, empirical estimates of $\Heritability$ are surprisingly stable across species and fall within the range of 0.2-0.5 in a vast majority of phenotypic traits tested~\mbox{%DIFAUXCMD
...
\DIFaddbegin \DIFadd{As such, the evidence for $\EstNI > 1$ does not necessarily imply that the trait is evolving under diversifying selection since non-neutral markers included in the estimate of divergence can lead to a spurious $\EstNI > 1$ (see table~\ref{table:assumptions}).
}\DIFaddend 
...
Under stabilizing selection, the variation between species is depleted because the mean trait \DIFdelbegin \DIFdel{value is maintained similar }\DIFdelend \DIFaddbegin \DIFadd{is maintained toward similar values }\DIFaddend between different species, which leads to $\NI < 1$.
...
\DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{An assumption of our test is that the neutral phenotypic trait is evolving as a Brownian process and is, therefore, unbounded.
...
.
However, this method does not formally allow testing for diversifying selection, and requires many traits such as expression level data to seek outliers genes}\DIFaddend ~\citep{rohlfs_phylogenetic_2015, gillard_comparative_2021}.
...
.
Specifically, by leveraging estimates of effective population size ($\Ne$) and number of generations between species, or alternatively by assuming their constancy, these methods can }\DIFaddend test for departures from the null model of neutral evolution for a single \DIFdelbegin \DIFdel{test.
...
.
However, $\Ne$ and the number of generations are complex parameters to correctly infer, and is usually done for a pair or only a few species, and ultimately requires large genomic datasets and heavy statistical methods~\mbox{%DIFAUXCMD
...
In \DIFdelbegin \DIFdel{the contextof within species variation, }\DIFdelend \DIFaddbegin \DIFadd{this context, our test bears many similarities to }\DIFaddend \QstFst\ tests \DIFaddbegin \DIFadd{(and their derivatives) that }\DIFaddend have been developed to \DIFdelbegin \DIFdel{compare trait and sequence }\DIFdelend \DIFaddbegin \DIFadd{test for selection of a trait }\DIFaddend across several populations \DIFdelbegin \DIFdel{to test for selection~\mbox{%DIFAUXCMD
...
\citep{martin_multivariate_2008, leinonen_qst_2013} }\hskip0pt%DIFAUXCMD
or co-ancestry between individuals~\mbox{%DIFAUXCMD
\citep{ovaskainen_new_2011}}\hskip0pt%DIFAUXCMD
.
...
As an example, trait variation is often modeled as a Brownian process running on a time-calibrated tree, which can produce biases~\mbox{%DIFAUXCMD
...
We argue, that the soundness of studying trait evolution on $d$-scaled trees can be evaluated by the absolute fit of a model to the data~\mbox{%DIFAUXCMD
...
First, we acknowledge that our test took inspiration from the \citet{mcdonald_adaptative_1991} test devised for protein-coding DNA sequences \DIFdelbegin \DIFdel{, where synonymous mutations were used to determine the neutral expectation, and the inflation of divergence was compared to polymorphism within species}\DIFdelend \DIFaddbegin \DIFadd{in a pair of species, except that the non-synonymous versus synonymous distinction is replaced by the comparison between quantitative trait and neutral genomic sequence}\DIFaddend .
Second, \DIFdelbegin \DIFdel{because $\NI$ was compared to 1, our test ultimately bear analogy to the }\DIFdelend \DIFaddbegin \DIFadd{at the phylogenetic scale, when comparison is done across several species, our test also bears analogy to }\DIFaddend codon-based test of selection, where the ratio of non-synonymous to synonymous substitutions ($\dnds$) is compared to 1~\citep{goldman_codonbased_1994, muse_likelihood_1994}.
...
\DIFaddend 
    ...
    \DIFaddbegin \bibitem[Anisimova and Kosiol(2009)Anisimova and
    Kosiol]{anisimova_investigating_2009}
    \DIFadd{Anisimova, M. and Kosiol, C. 2009.
    }\newblock \DIFadd{Investigating protein-coding sequence evolution with probabilistic
    codon substitution models.
    }\newblock {\em \DIFadd{Molecular Biology and Evolution\/}}\DIFadd{, }{\DIFadd{26}}\DIFadd{(2): 255--271.
}

    \bibitem[Arnold {\em et~al.}(2008)Arnold, B{\"u}rger, Hohenlohe, Ajie, and
    Jones]{arnold_understanding_2008}
    \DIFadd{Arnold, S.~J., B}{\DIFadd{\"u}}\DIFadd{rger, R., Hohenlohe, P.~A., Ajie, B.~C., and Jones, A.~G.
    2008.
    }\newblock \DIFadd{Understanding the evolution and stability of the }{{\DIFadd{G-Matrix}}}\DIFadd{.
    }\newblock {\em \DIFadd{Evolution\/}}\DIFadd{, }{\DIFadd{62}}\DIFadd{(10): 2451--2461.
}

    \bibitem[Barton {\em et~al.}(2017)Barton, Etheridge, and
    V{\'e}ber]{barton_infinitesimal_2017}
    \DIFadd{Barton, N.~H., Etheridge, A.~M., and V}{\DIFadd{\'e}}\DIFadd{ber, A. 2017.
    }\newblock \DIFadd{The infinitesimal model: }{{\DIFadd{Definition}}}\DIFadd{, derivation, and
    implications.
    }\newblock {\em \DIFadd{Theoretical Population Biology\/}}\DIFadd{, }{\DIFadd{118}}\DIFadd{: 50--73.
}
    ...
    \DIFadd{Bergeron, L.~A., Besenbacher, S., Zheng, J., Li, P., Bertelsen, M.~F.,
    Quintard, B., Hoffman, J.~I., Li, Z., St.~Leger, J., Shao, C., Stiller, J.,
    Gilbert, M. T.~P., Schierup, M.~H., and Zhang, G. 2023.
    }\newblock \DIFadd{Evolution of the germline mutation rate across vertebrates.
    }\newblock {\em \DIFadd{Nature\/}}\DIFadd{, pages 1--7.
}

    \bibitem[Blanquart and Bataillon(2016)Blanquart and
    Bataillon]{blanquart_epistasis_2016}
    \DIFadd{Blanquart, F. and Bataillon, T. 2016.
    }\newblock \DIFadd{Epistasis and the structure of fitness landscapes: }{{\DIFadd{Are}}}
    \DIFadd{experimental fitness landscapes compatible with fisher's geometric model?
    }\newblock {\em \DIFadd{Genetics\/}}\DIFadd{, }{\DIFadd{203}}\DIFadd{(2): 847--862.
}

    \bibitem[Catal{\'a}n {\em et~al.}(2019)Catal{\'a}n, Briscoe, and
    H{\"o}hna]{catalan_drift_2019}
    \DIFadd{Catal}{\DIFadd{\'a}}\DIFadd{n, A., Briscoe, A.~D., and H}{\DIFadd{\"o}}\DIFadd{hna, S. 2019.
    }\newblock \DIFadd{Drift and directional selection are the evolutionary forces driving
    gene expression divergence in eye and brain tissue of }{{\DIFadd{Heliconius}}}
    \DIFadd{butterflies.
    }\newblock {\em \DIFadd{Genetics\/}}\DIFadd{, }{\DIFadd{213}}\DIFadd{(2): 581--594.
}

    \bibitem[Cooper {\em et~al.}(2016)Cooper, Thomas, Venditti, Meade, and
    Freckleton]{cooper_cautionary_2016}
    \DIFadd{Cooper, N., Thomas, G.~H., Venditti, C., Meade, A., and Freckleton, R.~P. 2016.
    }\newblock \DIFadd{A cautionary note on the use of }{{\DIFadd{Ornstein Uhlenbeck}}} \DIFadd{models in
    macroevolutionary studies.
    }\newblock {\em \DIFadd{Biological Journal of the Linnean Society\/}}\DIFadd{, }{\DIFadd{118}}\DIFadd{(1): 64--77.
}

    \bibitem[Crow(2010)Crow]{crow_epistasis_2010}
    \DIFadd{Crow, J.~F. 2010.
    }\newblock \DIFadd{On epistasis: Why it is unimportant in polygenic directional
    selection.
    }\newblock {\em \DIFadd{Philosophical Transactions of the Royal Society B: Biological
    Sciences\/}}\DIFadd{, }{\DIFadd{365}}\DIFadd{(1544): 1241--1244.
}

    \bibitem[Edelaar {\em et~al.}(2011)Edelaar, Burraco, and
        {Gomez-Mestre}]{edelaar_comparisons_2011}
    \DIFadd{Edelaar, P., Burraco, P., and }{\DIFadd{Gomez-Mestre}}\DIFadd{, I. 2011.
    }\newblock \DIFadd{Comparisons between }{{\DIFadd{QST}}} \DIFadd{and }{{\DIFadd{FST}}}\DIFadd{---how wrong have we been?
    }\newblock {\em \DIFadd{Molecular Ecology\/}}\DIFadd{, }{\DIFadd{20}}\DIFadd{(23): 4830--4839.
}

    \bibitem[Felsenstein(1985)Felsenstein]{felsenstein_phylogenies_1985}
    \DIFadd{Felsenstein, J. 1985.
    }\newblock \DIFadd{Phylogenies and the }{{\DIFadd{Comparative Method}}}\DIFadd{.
    }\newblock {\em \DIFadd{The American Naturalist\/}}\DIFadd{, }{\DIFadd{125}}\DIFadd{(1): 1--15.
}

    \bibitem[Felsenstein(2008)Felsenstein]{felsenstein_comparative_2008}
    \DIFadd{Felsenstein, J. 2008.
    }\newblock \DIFadd{Comparative methods with sampling error and within-species variation:
    Contrasts revisited and revised.
    }\newblock {\em \DIFadd{The American Naturalist\/}}\DIFadd{, }{\DIFadd{171}}\DIFadd{(6): 713--725.
}
    ...
    \DIFadd{Foley, N.~M., Mason, V.~C., Harris, A.~J., Bredemeyer, K.~R., Damas, J., Lewin,
    H.~A., Eizirik, E., Gatesy, J., Karlsson, E.~K., }{\DIFadd{Lindblad-Toh}}\DIFadd{, K.,
        }{\DIFadd{Zoonomia Consortium}}\DIFadd{, Springer, M.~S., and Murphy, W.~J. 2023.
    }\newblock \DIFadd{A genomic timescale for placental mammal evolution.
    }\newblock {\em \DIFadd{Science\/}}\DIFadd{, }{\DIFadd{380}}\DIFadd{(6643): eabl8189.
}

    \bibitem[Fraser(2020)Fraser]{fraser_detecting_2020}
    \DIFadd{Fraser, H.~B. 2020.
    }\newblock \DIFadd{Detecting selection with a genetic cross.
    }\newblock {\em \DIFadd{Proceedings of the National Academy of Sciences\/}}\DIFadd{, }{\DIFadd{117}}\DIFadd{(36):
    22323--22330.
}

    \bibitem[Gaboriau {\em et~al.}(2020)Gaboriau, Mendes, Joly, Silvestro, and
    Salamin]{gaboriau_multiplatform_2020}
    \DIFadd{Gaboriau, T., Mendes, F.~K., Joly, S., Silvestro, D., and Salamin, N. 2020.
    }\newblock \DIFadd{A multi-platform package for the analysis of intra- and interspecific
    trait evolution.
    }\newblock {\em \DIFadd{Methods in Ecology and Evolution\/}}\DIFadd{, }{\DIFadd{11}}\DIFadd{(11): 1439--1447.
}

    \bibitem[Gaboriau {\em et~al.}(2023)Gaboriau, Tobias, Silvestro, and
    Salamin]{gaboriau_exploring_2023}
    \DIFadd{Gaboriau, T., Tobias, J.~A., Silvestro, D., and Salamin, N. 2023.
    }\newblock \DIFadd{Exploring the }{{\DIFadd{Macroevolutionary Signature}}} \DIFadd{of }{{\DIFadd{Asymmetric
    Inheritance}}} \DIFadd{at }{{\DIFadd{Speciation}}}\DIFadd{.
}
    ...
    \DIFadd{Genereux, D.~P., Serres, A., Armstrong, J., Johnson, J., Marinescu, V.~D.,
    Mur}{\DIFadd{\'e}}\DIFadd{n, E., Juan, D., Bejerano, G., Casewell, N.~R., Chemnick, L.~G.,
    ...
    Swofford, R., Taipale, J., Teeling, E.~C., }{\DIFadd{Turner-Maier}}\DIFadd{, J., Alfoldi, J.,
    Birren, B., Ryder, O.~A., Lewin, H.~A., Paten, B., }{\DIFadd{Marques-Bonet}}\DIFadd{, T.,
        }{\DIFadd{Lindblad-Toh}}\DIFadd{, K., Karlsson, E.~K., and }{\DIFadd{Zoonomia Consortium}} \DIFadd{2020.
    }\newblock \DIFadd{A comparative genomics multitool for scientific discovery and
    conservation.
    }\newblock {\em \DIFadd{Nature\/}}\DIFadd{, }{\DIFadd{587}}\DIFadd{(7833): 240--245.
}
    ...
    \DIFadd{Gillard, G.~B., Gr}{\DIFadd{\o}}\DIFadd{nvold, L., R}{\DIFadd{\o}}\DIFadd{s}{\DIFadd{\ae}}\DIFadd{g, L.~L., Holen, M.~M., Monsen,
        }{\DIFadd{\O}}\DIFadd{., Koop, B.~F., Rondeau, E.~B., Gundappa, M.~K., Mendoza, J., Macqueen,
    D.~J., Rohlfs, R.~V., Sandve, S.~R., and Hvidsten, T.~R. 2021.
    }\newblock \DIFadd{Comparative regulomics supports pervasive selection on gene dosage
    following whole genome duplication.
    }\newblock {\em \DIFadd{Genome Biology\/}}\DIFadd{, }{\DIFadd{22}}\DIFadd{(1): 103.
}

    \bibitem[Goldman and Yang(1994)Goldman and Yang]{goldman_codonbased_1994}
    \DIFadd{Goldman, N. and Yang, Z. 1994.
    }\newblock \DIFadd{A codon-based model of nucleotide substitution for protein-coding
        }{{\DIFadd{DNA}}} \DIFadd{sequences.
    }\newblock {\em \DIFadd{Molecular Biology and Evolution\/}}\DIFadd{, }{\DIFadd{11}}\DIFadd{(5): 725--736.
}
    ...
    \DIFadd{Grabowski, M., Pienaar, J., Voje, K.~L., Andersson, S., }{\DIFadd{Fuentes-Gonz}{\DIFadd{\'a}}\DIFadd{lez}}\DIFadd{,
    J., Kopperud, B.~T., Moen, D.~S., Tsuboi, M., Uyeda, J., and Hansen, T.~F.
    2023.
    }\newblock \DIFadd{A }{{\DIFadd{Cautionary Note}}} \DIFadd{on ``}{{\DIFadd{A Cautionary Note}}} \DIFadd{on the }{{\DIFadd{Use}}} \DIFadd{of
        }{{\DIFadd{Ornstein Uhlenbeck Models}}} \DIFadd{in }{{\DIFadd{Macroevolutionary Studies}}}\DIFadd{''.
    }\newblock {\em \DIFadd{Systematic Biology\/}}\DIFadd{, }{\DIFadd{72}}\DIFadd{(4): 955--963.
}

    \bibitem[Hansen(1997)Hansen]{hansen_stabilizing_1997}
    \DIFadd{Hansen, T.~F. 1997.
    }\newblock \DIFadd{Stabilizing selection and the comparative analysis of adaptation.
    }\newblock {\em \DIFadd{Evolution\/}}\DIFadd{, }{\DIFadd{51}}\DIFadd{(5): 1341--1351.
}

    \bibitem[Hansen and Bartoszek(2012)Hansen and
    Bartoszek]{hansen_interpreting_2012}
    \DIFadd{Hansen, T.~F. and Bartoszek, K. 2012.
    }\newblock \DIFadd{Interpreting the evolutionary regression: The interplay between
    observational and biological errors in phylogenetic comparative studies.
    }\newblock {\em \DIFadd{Systematic Biology\/}}\DIFadd{, }{\DIFadd{61}}\DIFadd{(3): 413--425.
}

    \bibitem[Hansen and Martins(1996)Hansen and Martins]{hansen_translating_1996}
    \DIFadd{Hansen, T.~F. and Martins, E.~P. 1996.
    }\newblock \DIFadd{Translating between microevolutionary process and macroevolutionary
    patterns: The correlation structure of interspecific data.
    }\newblock {\em \DIFadd{Evolution\/}}\DIFadd{, }{\DIFadd{50}}\DIFadd{(4): 1404--1417.
}

    \bibitem[Hansen and P{\'e}labon(2021)Hansen and
    P{\'e}labon]{hansen_evolvability_2021}
    \DIFadd{Hansen, T.~F. and P}{\DIFadd{\'e}}\DIFadd{labon, C. 2021.
    }\newblock \DIFadd{Evolvability: }{{\DIFadd{A Quantitative-Genetics Perspective}}}\DIFadd{.
    }\newblock {\em \DIFadd{Annual Review of Ecology, Evolution, and Systematics\/}}\DIFadd{,
        }{\DIFadd{52}}\DIFadd{(1): 153--175.
}

    \bibitem[Hansen {\em et~al.}(2011)Hansen, P{\'e}labon, and
    Houle]{hansen_heritability_2011}
    \DIFadd{Hansen, T.~F., P}{\DIFadd{\'e}}\DIFadd{labon, C., and Houle, D. 2011.
    }\newblock \DIFadd{Heritability is not }{{\DIFadd{Evolvability}}}\DIFadd{.
    }\newblock {\em \DIFadd{Evolutionary Biology\/}}\DIFadd{, }{\DIFadd{38}}\DIFadd{(3): 258--277.
}

    \bibitem[Harmon(2018)Harmon]{harmon_phylogenetic_2018}
    \DIFadd{Harmon, L. 2018.
    }\newblock \DIFadd{Phylogenetic comparative methods: Learning from trees.
}

    \bibitem[Hill {\em et~al.}(2008)Hill, Goddard, and Visscher]{hill_data_2008}
    \DIFadd{Hill, W.~G., Goddard, M.~E., and Visscher, P.~M. 2008.
    }\newblock \DIFadd{Data and }{{\DIFadd{Theory Point}}} \DIFadd{to }{{\DIFadd{Mainly Additive Genetic Variance}}} \DIFadd{for
        }{{\DIFadd{Complex Traits}}}\DIFadd{.
    }\newblock {\em \DIFadd{PLOS Genetics\/}}\DIFadd{, }{\DIFadd{4}}\DIFadd{(2): e1000008.
}

    \bibitem[Hohenlohe and Arnold(2008)Hohenlohe and Arnold]{hohenlohe_mipod_2008}
    \DIFadd{Hohenlohe, P.~A. and Arnold, S.~J. 2008.
    }\newblock {{\DIFadd{MIPoD}}}\DIFadd{: }{{\DIFadd{A}}} \DIFadd{hypothesis-testing framework for microevolutionary
    inference from patterns of divergence.
    }\newblock {\em \DIFadd{The American Naturalist\/}}\DIFadd{, }{\DIFadd{171}}\DIFadd{(3): 366--385.
}

    \bibitem[Hu {\em et~al.}(2022)Hu, Park, and Reecy]{hu_bringing_2022}
    \DIFadd{Hu, Z.-L., Park, C.~A., and Reecy, J.~M. 2022.
    }\newblock \DIFadd{Bringing the }{{\DIFadd{Animal QTLdb}}} \DIFadd{and }{{\DIFadd{CorrDB}}} \DIFadd{into the future: Meeting
    new challenges and providing updated services.
    }\newblock {\em \DIFadd{Nucleic Acids Research\/}}\DIFadd{, }{\DIFadd{50}}\DIFadd{(D1): D956--D961.
}
    ...
    \DIFadd{Huber, B., Whibley, A., Poul, Y.~L., Navarro, N., Martin, A., Baxter, S., Shah,
    A., Gilles, B., Wirth, T., McMillan, W.~O., and Joron, M. 2015.
    }\newblock \DIFadd{Conservatism and novelty in the genetic architecture of adaptation in
        }{{\DIFadd{Heliconius}}} \DIFadd{butterflies.
    }\newblock {\em \DIFadd{Heredity\/}}\DIFadd{, }{\DIFadd{114}}\DIFadd{(5): 515--524.
}

    \bibitem[Huelsenbeck and Rannala(2003)Huelsenbeck and
    Rannala]{huelsenbeck_detecting_2003}
    \DIFadd{Huelsenbeck, J.~P. and Rannala, B. 2003.
    }\newblock \DIFadd{Detecting correlation between characters in a comparative analysis
    with uncertain phylogeny.
    }\newblock {\em \DIFadd{Evolution\/}}\DIFadd{, }{\DIFadd{57}}\DIFadd{(6): 1237--1247.
}

    \bibitem[Ingram and Mahler(2013)Ingram and Mahler]{ingram_surface_2013}
    \DIFadd{Ingram, T. and Mahler, D. 2013.
    }\newblock {{\DIFadd{SURFACE}}}\DIFadd{: Detecting convergent evolution from comparative data by
    fitting }{{\DIFadd{Ornstein-Uhlenbeck}}} \DIFadd{models with stepwise }{{\DIFadd{Akaike Information
    Criterion}}}\DIFadd{.
    }\newblock {\em \DIFadd{Methods in Ecology and Evolution\/}}\DIFadd{, }{\DIFadd{4}}\DIFadd{(5): 416--425.
}

    \bibitem[Jensen {\em et~al.}(2019)Jensen, Payseur, Stephan, Aquadro, Lynch,
        Charlesworth, and Charlesworth]{jensen_importance_2019}
    \DIFadd{Jensen, J.~D., Payseur, B.~A., Stephan, W., Aquadro, C.~F., Lynch, M.,
    Charlesworth, D., and Charlesworth, B. 2019.
    }\newblock \DIFadd{The importance of the }{{\DIFadd{Neutral Theory}}} \DIFadd{in 1968 and 50 years on:
        }{{\DIFadd{A}}} \DIFadd{response to }{{\DIFadd{Kern}}} \DIFadd{and }{{\DIFadd{Hahn}}} \DIFadd{2018.
    }\newblock {\em \DIFadd{Evolution\/}}\DIFadd{, }{\DIFadd{73}}\DIFadd{(1): 111--114.
}

    \bibitem[Khabbazian {\em et~al.}(2016)Khabbazian, Kriebel, Rohe, and
    An{\'e}]{khabbazian_fast_2016}
    \DIFadd{Khabbazian, M., Kriebel, R., Rohe, K., and An}{\DIFadd{\'e}}\DIFadd{, C. 2016.
    }\newblock \DIFadd{Fast and accurate detection of evolutionary shifts in
        }{{\DIFadd{Ornstein}}}\DIFadd{--}{{\DIFadd{Uhlenbeck}}} \DIFadd{models.
    }\newblock {\em \DIFadd{Methods in Ecology and Evolution\/}}\DIFadd{, }{\DIFadd{7}}\DIFadd{(7): 811--824.
}
    ...
    \DIFadd{Khaitovich, P., Weiss, G., Lachmann, M., Hellmann, I., Enard, W., Muetzel, B.,
    Wirkner, U., Ansorge, W., and P}{\DIFadd{\"a}}{\DIFadd{\"a}}\DIFadd{bo, S. 2004.
    }\newblock \DIFadd{A neutral model of transcriptome evolution.
    }\newblock {\em \DIFadd{PLOS Biology\/}}\DIFadd{, }{\DIFadd{2}}\DIFadd{(5): e132.
}

    \bibitem[Kimura(1962)Kimura]{kimura_probability_1962}
    \DIFadd{Kimura, M. 1962.
    }\newblock \DIFadd{On the probability of fixation of mutant genes in a population.
    }\newblock {\em \DIFadd{Genetics\/}}\DIFadd{, }{\DIFadd{47}}\DIFadd{(6): 713--719.
}

    \bibitem[Kimura(1968)Kimura]{kimura_evolutionary_1968}
    \DIFadd{Kimura, M. 1968.
    }\newblock \DIFadd{Evolutionary rate at the molecular level.
    }\newblock {\em \DIFadd{Nature\/}}\DIFadd{, }{\DIFadd{217}}\DIFadd{(5129): 624--626.
}

    \bibitem[Kostikova {\em et~al.}(2016)Kostikova, Silvestro, Pearman, and
    Salamin]{kostikova_bridging_2016}
    \DIFadd{Kostikova, A., Silvestro, D., Pearman, P.~B., and Salamin, N. 2016.
    }\newblock \DIFadd{Bridging inter- and intraspecific trait evolution with a hierarchical
    bayesian approach.
    }\newblock {\em \DIFadd{Systematic Biology\/}}\DIFadd{, }{\DIFadd{65}}\DIFadd{(3): 417--431.
}
    ...
    \DIFadd{Kuderna, L. F.~K., Gao, H., Janiak, M.~C., Kuhlwilm, M., Orkin, J.~D.,
    Bataillon, T., Manu, S., Valenzuela, A., Bergman, J., Rousselle, M., Silva,
    F.~E., Agueda, L., Blanc, J., Gut, M., }{\DIFadd{de Vries}}\DIFadd{, D., Goodhead, I., Harris,
    R.~A., Raveendran, M., Jensen, A., Chuma, I.~S., Horvath, J.~E., Hvilsom, C.,
    Juan, D., Frandsen, P., Schraiber, J.~G., }{\DIFadd{de Melo}}\DIFadd{, F.~R., Bertuol, F.,
    Byrne, H., Sampaio, I., Farias, I., Valsecchi, J., Messias, M., }{\DIFadd{da Silva}}\DIFadd{,
    M. N.~F., Trivedi, M., Rossi, R., Hrbek, T., Andriaholinirina, N.,
    Rabarivola, C.~J., Zaramody, A., Jolly, C.~J., }{\DIFadd{Phillips-Conroy}}\DIFadd{, J.,
    Wilkerson, G., Abee, C., Simmons, J.~H., }{\DIFadd{Fernandez-Duque}}\DIFadd{, E., Kanthaswamy,
    ...
    }\newblock \DIFadd{A global catalog of whole-genome diversity from 233 primate species.
    }\newblock {\em \DIFadd{Science\/}}\DIFadd{, }{\DIFadd{380}}\DIFadd{(6648): 906--913.
}

    \bibitem[Lamy {\em et~al.}(2012)Lamy, Plomion, Kremer, and
    Delzon]{lamy_qst_2012}
    \DIFadd{Lamy, J.-B., Plomion, C., Kremer, A., and Delzon, S. 2012.
    }\newblock {{\DIFadd{QST}}} {\DIFadd{$<$}} {{\DIFadd{FST As}}} \DIFadd{a signature of canalization.
    }\newblock {\em \DIFadd{Molecular Ecology\/}}\DIFadd{, }{\DIFadd{21}}\DIFadd{(23): 5646--5655.
}

    \bibitem[Lande(1979)Lande]{lande_quantitative_1979}
    \DIFadd{Lande, R. 1979.
    }\newblock \DIFadd{Quantitative genetic analysis of multivariate evolution, applied to
    brain: Body size allometry.
    }\newblock {\em \DIFadd{Evolution\/}}\DIFadd{, }{\DIFadd{33}}\DIFadd{(1): 402--416.
}

    \bibitem[Lande(1980a)Lande]{lande_genetic_1980}
    \DIFadd{Lande, R. 1980a.
    }\newblock \DIFadd{Genetic variation and phenotypic evolution during allopatric
    speciation.
    }\newblock {\em \DIFadd{The American Naturalist\/}}\DIFadd{, }{\DIFadd{116}}\DIFadd{(4): 463--479.
}

    \bibitem[Lande(1980b)Lande]{lande_sexual_1980}
    \DIFadd{Lande, R. 1980b.
    }\newblock \DIFadd{Sexual dimorphism, sexual selection, and adaptation in polygenic
    characters.
    }\newblock {\em \DIFadd{Evolution\/}}\DIFadd{, }{\DIFadd{34}}\DIFadd{(2): 292--305.
}

    \bibitem[Lartillot and Delsuc(2012)Lartillot and Delsuc]{lartillot_joint_2012}
    \DIFadd{Lartillot, N. and Delsuc, F. 2012.
    }\newblock \DIFadd{Joint reconstruction of divergence times and life-history evolution
    in placental mammals using a phylogenetic covariance model.
    }\newblock {\em \DIFadd{Evolution\/}}\DIFadd{, }{\DIFadd{66}}\DIFadd{(6): 1773--1787.
}

    \bibitem[Lartillot and Poujol(2011)Lartillot and
    Poujol]{lartillot_phylogenetic_2011}
    \DIFadd{Lartillot, N. and Poujol, R. 2011.
    }\newblock \DIFadd{A phylogenetic model for investigating correlated evolution of
    substitution rates and continuous phenotypic characters.
    }\newblock {\em \DIFadd{Molecular Biology and Evolution\/}}\DIFadd{, }{\DIFadd{28}}\DIFadd{(1): 729--744.
}

    \bibitem[Latrille {\em et~al.}(2021)Latrille, Lanore, and
    Lartillot]{latrille_inferring_2021}
    \DIFadd{Latrille, T., Lanore, V., and Lartillot, N. 2021.
    }\newblock \DIFadd{Inferring long-term effective population size with
    mutation--selection models.
    }\newblock {\em \DIFadd{Molecular Biology and Evolution\/}}\DIFadd{, }{\DIFadd{38}}\DIFadd{(10): 4573--4587.
}

    \bibitem[Latrille {\em et~al.}(2023)Latrille, Rodrigue, and
    Lartillot]{latrille_genes_2023}
    \DIFadd{Latrille, T., Rodrigue, N., and Lartillot, N. 2023.
    }\newblock \DIFadd{Genes and sites under adaptation at the phylogenetic scale also
    exhibit adaptation at the population-genetic scale.
    }\newblock {\em \DIFadd{Proceedings of the National Academy of Sciences of the United
    States of America\/}}\DIFadd{, }{\DIFadd{120}}\DIFadd{(11): e2214977120.
}

    \bibitem[Leinonen {\em et~al.}(2008)Leinonen, O'hara, Cano, and
    Meril{\"a}]{leinonen_comparative_2008}
    \DIFadd{Leinonen, T., O'hara, R.~B., Cano, J.~M., and Meril}{\DIFadd{\"a}}\DIFadd{, J. 2008.
    }\newblock \DIFadd{Comparative studies of quantitative trait and neutral marker
    divergence: A meta-analysis.
    }\newblock {\em \DIFadd{Journal of Evolutionary Biology\/}}\DIFadd{, }{\DIFadd{21}}\DIFadd{(1): 1--17.
}

    \bibitem[Leinonen {\em et~al.}(2013)Leinonen, McCairns, O'Hara, and
    Meril{\"a}]{leinonen_qst_2013}
    \DIFadd{Leinonen, T., McCairns, R. J.~S., O'Hara, R.~B., and Meril}{\DIFadd{\"a}}\DIFadd{, J. 2013.
    }\newblock {{\DIFadd{QST}}}\DIFadd{--}{{\DIFadd{FST}}} \DIFadd{comparisons: Evolutionary and ecological insights
    from genomic heterogeneity.
    }\newblock {\em \DIFadd{Nature Reviews Genetics\/}}\DIFadd{, }{\DIFadd{14}}\DIFadd{(3): 179--190.
}

    \bibitem[Lemos {\em et~al.}(2001)Lemos, Marroig, and
    Cerqueira]{lemos_evolutionary_2001}
    \DIFadd{Lemos, B., Marroig, G., and Cerqueira, R. 2001.
    }\newblock \DIFadd{Evolutionary rates and stabilizing selection in large-bodied opossum
    skulls (}{{\DIFadd{Didelphimorphia}}}\DIFadd{: }{{\DIFadd{Didelphidae}}}\DIFadd{).
    }\newblock {\em \DIFadd{Journal of Zoology\/}}\DIFadd{, }{\DIFadd{255}}\DIFadd{(2): 181--189.
}

    \bibitem[Lemos {\em et~al.}(2005)Lemos, Meiklejohn, C{\'a}ceres, and
    Hartl]{lemos_rates_2005}
    \DIFadd{Lemos, B., Meiklejohn, C.~D., C}{\DIFadd{\'a}}\DIFadd{ceres, M., and Hartl, D.~L. 2005.
    }\newblock \DIFadd{Rates of }{{\DIFadd{Divergence}}} \DIFadd{in }{{\DIFadd{Gene Expression Profiles}}} \DIFadd{of
        }{{\DIFadd{Primates}}}\DIFadd{, }{{\DIFadd{Mice}}}\DIFadd{, and }{{\DIFadd{Flies}}}\DIFadd{: }{{\DIFadd{Stabilizing Selection}}} \DIFadd{and
        }{{\DIFadd{Variability Among Functional Categories}}}\DIFadd{.
    }\newblock {\em \DIFadd{Evolution\/}}\DIFadd{, }{\DIFadd{59}}\DIFadd{(1): 126--137.
}

    \bibitem[Litsios and Salamin(2012)Litsios and Salamin]{litsios_effects_2012}
    \DIFadd{Litsios, G. and Salamin, N. 2012.
    }\newblock \DIFadd{Effects of }{{\DIFadd{Phylogenetic Signal}}} \DIFadd{on }{{\DIFadd{Ancestral State
    Reconstruction}}}\DIFadd{.
    }\newblock {\em \DIFadd{Systematic Biology\/}}\DIFadd{, }{\DIFadd{61}}\DIFadd{(3): 533--538.
}

    \bibitem[Louca and Pennell(2020)Louca and Pennell]{louca_extant_2020}
    \DIFadd{Louca, S. and Pennell, M.~W. 2020.
    }\newblock \DIFadd{Extant timetrees are consistent with a myriad of diversification
    histories.
    }\newblock {\em \DIFadd{Nature\/}}\DIFadd{, }{\DIFadd{580}}\DIFadd{(7804): 502--505.
}

    \bibitem[Lynch(1991)Lynch]{lynch_methods_1991}
    \DIFadd{Lynch, M. 1991.
    }\newblock \DIFadd{Methods for the analysis of comparative data in evolutionary biology.
    }\newblock {\em \DIFadd{Evolution\/}}\DIFadd{, }{\DIFadd{45}}\DIFadd{(5): 1065--1080.
}

    \bibitem[Lynch and Crease(1990)Lynch and Crease]{lynch_analysis_1990}
    \DIFadd{Lynch, M. and Crease, T.~J. 1990.
    }\newblock \DIFadd{The analysis of population survey data on }{{\DIFadd{DNA}}} \DIFadd{sequence variation.
    }\newblock {\em \DIFadd{Molecular Biology and Evolution\/}}\DIFadd{, }{\DIFadd{7}}\DIFadd{(4): 377--394.
}

    \bibitem[Lynch and Walsh(1998)Lynch and Walsh]{lynch_genetics_1998}
    \DIFadd{Lynch, M. and Walsh, B. 1998.
    }\newblock {\em \DIFadd{Genetics and Analysis of Quantitative Traits\/}}\DIFadd{, volume~1.
    }\newblock {\DIFadd{Sinauer Sunderland, MA}}\DIFadd{.
}

    \bibitem[Lynch {\em et~al.}(1998)Lynch, Latta, Hicks, and
    Giorgianni]{lynch_mutation_1998}
    \DIFadd{Lynch, M., Latta, L., Hicks, J., and Giorgianni, M. 1998.
    }\newblock \DIFadd{Mutation, selection, and the maintenance of life-history variation in
    a natural population.
    }\newblock {\em \DIFadd{Evolution\/}}\DIFadd{, }{\DIFadd{52}}\DIFadd{(3): 727--733.
}

    \bibitem[Machado {\em et~al.}(2022)Machado, Marroig, and
    Hubbe]{machado_preeminent_2022}
    \DIFadd{Machado, F.~A., Marroig, G., and Hubbe, A. 2022.
    }\newblock \DIFadd{The pre-eminent role of directional selection in generating extreme
    morphological change in glyptodonts (}{{\DIFadd{Cingulata}}}\DIFadd{; }{{\DIFadd{Xenarthra}}}\DIFadd{).
    }\newblock {\em \DIFadd{Proceedings of the Royal Society B: Biological Sciences\/}}\DIFadd{,
        }{\DIFadd{289}}\DIFadd{(1967): 20212521.
}

    \bibitem[Machado {\em et~al.}(2023)Machado, Mongle, Slater, Penna, Wisniewski,
        Soffin, Dutra, and Uyeda]{machado_using_2023}
    \DIFadd{Machado, F.~A., Mongle, C.~S., Slater, G., Penna, A., Wisniewski, A., Soffin,
    A., Dutra, V., and Uyeda, J.~C. 2023.
    }\newblock \DIFadd{Using developmental rules to align microevolution with
    macroevolution.
}

    \bibitem[Martin {\em et~al.}(2008)Martin, Chapuis, and
    Goudet]{martin_multivariate_2008}
    \DIFadd{Martin, G., Chapuis, E., and Goudet, J. 2008.
    }\newblock \DIFadd{Multivariate }{{\DIFadd{Qst}}}\DIFadd{--}{{\DIFadd{Fst Comparisons}}}\DIFadd{: }{{\DIFadd{A Neutrality Test}}} \DIFadd{for
    the }{{\DIFadd{Evolution}}} \DIFadd{of the }{{\DIFadd{G Matrix}}} \DIFadd{in }{{\DIFadd{Structured Populations}}}\DIFadd{.
    }\newblock {\em \DIFadd{Genetics\/}}\DIFadd{, }{\DIFadd{180}}\DIFadd{(4): 2135--2149.
}

    \bibitem[McCandlish and Stoltzfus(2014)McCandlish and
    Stoltzfus]{mccandlish_modeling_2014}
    \DIFadd{McCandlish, D.~M. and Stoltzfus, A. 2014.
    }\newblock \DIFadd{Modeling evolution using the probability of fixation: }{{\DIFadd{History}}} \DIFadd{and
    implications.
    }\newblock {\em \DIFadd{Quarterly Review of Biology\/}}\DIFadd{, }{\DIFadd{89}}\DIFadd{(3): 225--252.
}

    \bibitem[McDonald and Kreitman(1991)McDonald and
    Kreitman]{mcdonald_adaptative_1991}
    \DIFadd{McDonald, J.~H. and Kreitman, M. 1991.
    }\newblock \DIFadd{Adaptative protein evolution at }{{\DIFadd{Adh}}} \DIFadd{locus in }{{\DIFadd{Drosophila}}}\DIFadd{.
    }\newblock {\em \DIFadd{Nature\/}}\DIFadd{, }{\DIFadd{351}}\DIFadd{(6328): 652--654.
}

    \bibitem[Meril{\"a} and Crnokrak(2001)Meril{\"a} and
    Crnokrak]{merila_comparison_2001}
    \DIFadd{Meril}{\DIFadd{\"a}}\DIFadd{, J. and Crnokrak, P. 2001.
    }\newblock \DIFadd{Comparison of genetic differentiation at marker loci and quantitative
    traits.
    }\newblock {\em \DIFadd{Journal of Evolutionary Biology\/}}\DIFadd{, }{\DIFadd{14}}\DIFadd{(6): 892--903.
}

    \bibitem[Mitov {\em et~al.}(2020)Mitov, Bartoszek, Asimomitis, and
    Stadler]{mitov_fast_2020}
    \DIFadd{Mitov, V., Bartoszek, K., Asimomitis, G., and Stadler, T. 2020.
    }\newblock \DIFadd{Fast likelihood calculation for multivariate }{{\DIFadd{Gaussian}}}
    \DIFadd{phylogenetic models with shifts.
    }\newblock {\em \DIFadd{Theoretical Population Biology\/}}\DIFadd{, }{\DIFadd{131}}\DIFadd{: 66--78.
}

    \bibitem[Muse and Gaut(1994)Muse and Gaut]{muse_likelihood_1994}
    \DIFadd{Muse, S.~V. and Gaut, B.~S. 1994.
    }\newblock \DIFadd{A likelihood approach for comparing synonymous and nonsynonymous
    nucleotide substitution rates, with application to the chloroplast genome.
    }\newblock {\em \DIFadd{Molecular Biology and Evolution\/}}\DIFadd{, }{\DIFadd{1}}\DIFadd{(5): 715--724.
}

    \bibitem[Nielsen(2005)Nielsen]{nielsen_molecular_2005}
    \DIFadd{Nielsen, R. 2005.
    }\newblock \DIFadd{Molecular signatures of natural selection.
    }\newblock {\em \DIFadd{Annual Review of Genetics\/}}\DIFadd{, }{\DIFadd{39}}\DIFadd{(1): 197--218.
}

    \bibitem[O'Meara {\em et~al.}(2006)O'Meara, An{\'e}, Sanderson, and
    Wainwright]{omeara_testing_2006}
    \DIFadd{O'Meara, B.~C., An}{\DIFadd{\'e}}\DIFadd{, C., Sanderson, M.~J., and Wainwright, P.~C. 2006.
    }\newblock \DIFadd{Testing for }{{\DIFadd{Different Rates}}} \DIFadd{of }{{\DIFadd{Continuous Trait Evolution Using
    Likelihood}}}\DIFadd{.
    }\newblock {\em \DIFadd{Evolution\/}}\DIFadd{, }{\DIFadd{60}}\DIFadd{(5): 922--933.
}

    \bibitem[Ovaskainen {\em et~al.}(2011)Ovaskainen, Karhunen, Zheng, Arias, and
    Meril{\"a}]{ovaskainen_new_2011}
    \DIFadd{Ovaskainen, O., Karhunen, M., Zheng, C., Arias, J. M.~C., and Meril}{\DIFadd{\"a}}\DIFadd{, J.
    2011.
    }\newblock \DIFadd{A }{{\DIFadd{New Method}}} \DIFadd{to }{{\DIFadd{Uncover Signatures}}} \DIFadd{of }{{\DIFadd{Divergent}}} \DIFadd{and
        }{{\DIFadd{Stabilizing Selection}}} \DIFadd{in }{{\DIFadd{Quantitative Traits}}}\DIFadd{.
    }\newblock {\em \DIFadd{Genetics\/}}\DIFadd{, }{\DIFadd{189}}\DIFadd{(2): 621--632.
}

    \bibitem[Pennell {\em et~al.}(2015)Pennell, FitzJohn, Cornwell, and
    Harmon]{pennell_model_2015}
    \DIFadd{Pennell, M.~W., FitzJohn, R.~G., Cornwell, W.~K., and Harmon, L.~J. 2015.
    }\newblock \DIFadd{Model }{{\DIFadd{Adequacy}}} \DIFadd{and the }{{\DIFadd{Macroevolution}}} \DIFadd{of }{{\DIFadd{Angiosperm
    Functional Traits}}}\DIFadd{.
    }\newblock {\em \DIFadd{The American Naturalist\/}}\DIFadd{, }{\DIFadd{186}}\DIFadd{(2): E33--E50.
}

    \bibitem[Porto {\em et~al.}(2015)Porto, Sebasti{\~a}o, Pavan, VandeBerg,
        Marroig, and Cheverud]{porto_rate_2015}
    \DIFadd{Porto, A., Sebasti}{\DIFadd{\~a}}\DIFadd{o, H., Pavan, S.~E., VandeBerg, J.~L., Marroig, G., and
    Cheverud, J.~M. 2015.
    }\newblock \DIFadd{Rate of evolutionary change in cranial morphology of the marsupial
    genus }{{\DIFadd{Monodelphis}}} \DIFadd{is constrained by the availability of additive genetic
    variation.
    }\newblock {\em \DIFadd{Journal of Evolutionary Biology\/}}\DIFadd{, }{\DIFadd{28}}\DIFadd{(4): 973--985.
}

    \bibitem[Price {\em et~al.}(2022)Price, Palmer~Droguett, Taylor, Kim, Place,
        Rogers, Mank, Cooney, and Wright]{price_detecting_2022}
    \DIFadd{Price, P.~D., Palmer~Droguett, D.~H., Taylor, J.~A., Kim, D.~W., Place, E.~S.,
    Rogers, T.~F., Mank, J.~E., Cooney, C.~R., and Wright, A.~E. 2022.
    }\newblock \DIFadd{Detecting signatures of selection on gene expression.
    }\newblock {\em \DIFadd{Nature Ecology \& Evolution\/}}\DIFadd{, pages 1--11.
}

    \bibitem[Pujol {\em et~al.}(2008)Pujol, Wilson, Ross, and
    Pannell]{pujol_are_2008}
    \DIFadd{Pujol, B., Wilson, A.~J., Ross, R. I.~C., and Pannell, J.~R. 2008.
    }\newblock \DIFadd{Are }{{\DIFadd{QST}}}\DIFadd{--}{{\DIFadd{FST}}} \DIFadd{comparisons for natural populations meaningful?
    }\newblock {\em \DIFadd{Molecular Ecology\/}}\DIFadd{, }{\DIFadd{17}}\DIFadd{(22): 4782--4785.
}

    \bibitem[Rohlfs and Nielsen(2015)Rohlfs and Nielsen]{rohlfs_phylogenetic_2015}
    \DIFadd{Rohlfs, R.~V. and Nielsen, R. 2015.
    }\newblock \DIFadd{Phylogenetic }{{\DIFadd{ANOVA}}}\DIFadd{: }{{\DIFadd{The}}} \DIFadd{expression variance and evolution
    model for quantitative trait evolution.
    }\newblock {\em \DIFadd{Systematic Biology\/}}\DIFadd{, }{\DIFadd{64}}\DIFadd{(5): 695--708.
}

    \bibitem[Rohlfs {\em et~al.}(2014)Rohlfs, Harrigan, and
    Nielsen]{rohlfs_modeling_2014}
    \DIFadd{Rohlfs, R.~V., Harrigan, P., and Nielsen, R. 2014.
    }\newblock \DIFadd{Modeling gene expression evolution with an extended
        }{{\DIFadd{Ornstein}}}\DIFadd{--}{{\DIFadd{Uhlenbeck}}} \DIFadd{process accounting for within-species variation.
    }\newblock {\em \DIFadd{Molecular Biology and Evolution\/}}\DIFadd{, }{\DIFadd{31}}\DIFadd{(1): 201--211.
}
    ...
    \DIFadd{Rolland, J., }{\DIFadd{Henao-Diaz}}\DIFadd{, L.~F., Doebeli, M., Germain, R., Harmon, L.~J.,
    Knowles, L.~L., Liow, L.~H., Mank, J.~E., Machac, A., Otto, S.~P., Pennell,
    M., Salamin, N., Silvestro, D., Sugawara, M., Uyeda, J., Wagner, C.~E., and
    Schluter, D. 2023.
    }\newblock \DIFadd{Conceptual and empirical bridges between micro- and macroevolution.
    }\newblock {\em \DIFadd{Nature Ecology \& Evolution\/}}\DIFadd{, pages 1--13.
}

    \bibitem[Schroeder and {von Cramon-Taubadel}(2017)Schroeder and {von
    Cramon-Taubadel}]{schroeder_evolution_2017}
    \DIFadd{Schroeder, L. and }{\DIFadd{von Cramon-Taubadel}}\DIFadd{, N. 2017.
    }\newblock \DIFadd{The evolution of hominoid cranial diversity: }{{\DIFadd{A}}} \DIFadd{quantitative
    genetic approach.
    }\newblock {\em \DIFadd{Evolution\/}}\DIFadd{, }{\DIFadd{71}}\DIFadd{(11): 2634--2649.
}

    \bibitem[Sella and Barton(2019)Sella and Barton]{sella_thinking_2019}
    \DIFadd{Sella, G. and Barton, N.~H. 2019.
    }\newblock \DIFadd{Thinking about the evolution of complex traits in the era of
    genome-wide association studies.
    }\newblock {\em \DIFadd{Annual Review of Genomics and Human Genetics\/}}\DIFadd{, }{\DIFadd{20}}\DIFadd{(1):
    461--493.
}

    \bibitem[Silvestro {\em et~al.}(2015)Silvestro, Kostikova, Litsios, Pearman,
        and Salamin]{silvestro_measurement_2015}
    \DIFadd{Silvestro, D., Kostikova, A., Litsios, G., Pearman, P.~B., and Salamin, N.
    2015.
    }\newblock \DIFadd{Measurement errors should always be incorporated in phylogenetic
    comparative analysis.
    }\newblock {\em \DIFadd{Methods in Ecology and Evolution\/}}\DIFadd{, }{\DIFadd{6}}\DIFadd{(3): 340--346.
}
    ...
    \DIFadd{Silvestro, D., Tejedor, M.~F., }{\DIFadd{Serrano-Serrano}}\DIFadd{, M.~L., Loiseau, O., Rossier,
    V., Rolland, J., Zizka, A., H}{\DIFadd{\"o}}\DIFadd{hna, S., Antonelli, A., and Salamin, N.
    2019.
    }\newblock \DIFadd{Early }{{\DIFadd{Arrival}}} \DIFadd{and }{{\DIFadd{Climatically-Linked Geographic Expansion}}} \DIFadd{of
        }{{\DIFadd{New World Monkeys}}} \DIFadd{from }{{\DIFadd{Tiny African Ancestors}}}\DIFadd{.
    }\newblock {\em \DIFadd{Systematic Biology\/}}\DIFadd{, }{\DIFadd{68}}\DIFadd{(1): 78--92.
}

    \bibitem[Simons {\em et~al.}(2018)Simons, Bullaughey, Hudson, and
    Sella]{simons_population_2018}
    \DIFadd{Simons, Y.~B., Bullaughey, K., Hudson, R.~R., and Sella, G. 2018.
    }\newblock \DIFadd{A population genetic interpretation of }{{\DIFadd{GWAS}}} \DIFadd{findings for human
    quantitative traits.
    }\newblock {\em \DIFadd{PLOS Biology\/}}\DIFadd{, }{\DIFadd{16}}\DIFadd{(3): e2002985.
}

    \bibitem[Tajima(1989)Tajima]{tajima_statistical_1989}
    \DIFadd{Tajima, F. 1989.
    }\newblock \DIFadd{Statistical method for testing the neutral mutation hypothesis by
        }{{\DIFadd{DNA}}} \DIFadd{polymorphism.
    }\newblock {\em \DIFadd{Genetics\/}}\DIFadd{, }{\DIFadd{123}}\DIFadd{(3): 585--595.
}

    \bibitem[Tenaillon(2014)Tenaillon]{tenaillon_utility_2014}
    \DIFadd{Tenaillon, O. 2014.
    }\newblock \DIFadd{The utility of fisher's geometric model in evolutionary genetics.
    }\newblock {\em \DIFadd{Annual Review of Ecology, Evolution, and Systematics\/}}\DIFadd{,
        }{\DIFadd{45}}\DIFadd{(1): 179--201.
}
    ...
    \DIFadd{Tsuboi, M., }{\DIFadd{van der Bijl}}\DIFadd{, W., Kopperud, B.~T., Erritz}{\DIFadd{\o}}\DIFadd{e, J., Voje, K.~L.,
    Kotrschal, A., Yopak, K.~E., Collin, S.~P., Iwaniuk, A.~N., and Kolm, N.
    2018.
    }\newblock \DIFadd{Breakdown of brain--body allometry and the encephalization of birds
    and mammals.
    }\newblock {\em \DIFadd{Nature Ecology \& Evolution\/}}\DIFadd{, }{\DIFadd{2}}\DIFadd{(9): 1492--1500.
}

    \bibitem[Tung {\em et~al.}(2015)Tung, Zhou, Alberts, Stephens, and
    Gilad]{tung_genetic_2015}
    \DIFadd{Tung, J., Zhou, X., Alberts, S.~C., Stephens, M., and Gilad, Y. 2015.
    }\newblock \DIFadd{The genetic architecture of gene expression levels in wild baboons.
    }\newblock {\em \DIFadd{eLife\/}}\DIFadd{, }{\DIFadd{4}}\DIFadd{: e04729.
}

    \bibitem[Turelli(1984)Turelli]{turelli_heritable_1984}
    \DIFadd{Turelli, M. 1984.
    }\newblock \DIFadd{Heritable genetic variation via mutation-selection balance:
        }{{\DIFadd{Lerch}}}\DIFadd{'s zeta meets the abdominal bristle.
    }\newblock {\em \DIFadd{Theoretical Population Biology\/}}\DIFadd{, }{\DIFadd{25}}\DIFadd{(2): 138--193.
}

    \bibitem[Turelli(2017)Turelli]{turelli_commentary_2017}
    \DIFadd{Turelli, M. 2017.
    }\newblock \DIFadd{Commentary: }{{\DIFadd{Fisher}}}\DIFadd{'s infinitesimal model: }{{\DIFadd{A}}} \DIFadd{story for the
    ages.
    }\newblock {\em \DIFadd{Theoretical Population Biology\/}}\DIFadd{, }{\DIFadd{118}}\DIFadd{: 46--49.
}

    \bibitem[Uyeda and Harmon(2014)Uyeda and Harmon]{uyeda_novel_2014}
    \DIFadd{Uyeda, J.~C. and Harmon, L.~J. 2014.
    }\newblock \DIFadd{A }{{\DIFadd{Novel Bayesian Method}}} \DIFadd{for }{{\DIFadd{Inferring}}} \DIFadd{and }{{\DIFadd{Interpreting}}}
    \DIFadd{the }{{\DIFadd{Dynamics}}} \DIFadd{of }{{\DIFadd{Adaptive Landscapes}}} \DIFadd{from }{{\DIFadd{Phylogenetic Comparative
    Data}}}\DIFadd{.
    }\newblock {\em \DIFadd{Systematic Biology\/}}\DIFadd{, }{\DIFadd{63}}\DIFadd{(6): 902--918.
}

    \bibitem[Uyeda {\em et~al.}(2011)Uyeda, Hansen, Arnold, and
    Pienaar]{uyeda_millionyear_2011}
    \DIFadd{Uyeda, J.~C., Hansen, T.~F., Arnold, S.~J., and Pienaar, J. 2011.
    }\newblock \DIFadd{The million-year wait for macroevolutionary bursts.
    }\newblock {\em \DIFadd{Proceedings of the National Academy of Sciences\/}}\DIFadd{, }{\DIFadd{108}}\DIFadd{(38):
    15908--15913.
}

    \bibitem[Walsh and Lynch(2018)Walsh and Lynch]{walsh_evolution_2018}
    \DIFadd{Walsh, B. and Lynch, M. 2018.
    }\newblock {\em \DIFadd{Evolution and }{{\DIFadd{Selection}}} \DIFadd{of }{{\DIFadd{Quantitative Traits}}}\DIFadd{\/}}\DIFadd{.
    }\newblock {\DIFadd{Oxford University Press}}\DIFadd{.
}

    \bibitem[Weaver {\em et~al.}(2007)Weaver, Roseman, and
    Stringer]{weaver_were_2007}
    \DIFadd{Weaver, T.~D., Roseman, C.~C., and Stringer, C.~B. 2007.
    }\newblock \DIFadd{Were neandertal and modern human cranial differences produced by
    natural selection or genetic drift?
    }\newblock {\em \DIFadd{Journal of Human Evolution\/}}\DIFadd{, }{\DIFadd{53}}\DIFadd{(2): 135--145.
}
    ...
    \DIFadd{Wilder, A.~P., Supple, M.~A., Subramanian, A., Mudide, A., Swofford, R.,
        }{\DIFadd{Serres-Armero}}\DIFadd{, A., Steiner, C., Koepfli, K.-P., Genereux, D.~P., Karlsson,
    E.~K., }{\DIFadd{Lindblad-Toh}}\DIFadd{, K., }{\DIFadd{Marques-Bonet}}\DIFadd{, T., Munoz~Fuentes, V., Foley, K.,
    Meyer, W.~K., }{\DIFadd{Zoonomia Consortium}}\DIFadd{, Ryder, O.~A., and Shapiro, B. 2023.
    }\newblock \DIFadd{The contribution of historical processes to contemporary extinction
    risk in placental mammals.
    }\newblock {\em \DIFadd{Science\/}}\DIFadd{, }{\DIFadd{380}}\DIFadd{(6643): eabn5856.
}
...
\begin{itemize}
    ...
    \item \DIFdelbegin \DIFdel{$g_{i,l} \in \{0, 1, 2\}$ }\DIFdelend \DIFaddbegin \DIFadd{$g_{\Indiv,l} \in \{0, 1, 2\}$ }\DIFaddend is the genotypic value at locus $l$ for individual \DIFdelbegin \DIFdel{$i \in \{1, \hdots, \Ne\}$}\DIFdelend \DIFaddbegin \DIFadd{$\Indiv \in \{1, \hdots, \Ne\}$}\DIFaddend .
    \item \DIFdelbegin \DIFdel{$G_i = \sum_{l=1}^{\NbrLoci} a_l \times g_{i,l}$ }\DIFdelend \DIFaddbegin \DIFadd{$G_{\Indiv} = \sum_{l=1}^{\NbrLoci} a_l \times g_{\Indiv,l}$ }\DIFaddend is the genotypic value for individual \DIFdelbegin \DIFdel{$i$}\DIFdelend \DIFaddbegin \DIFadd{$\Indiv$}\DIFaddend .
    \item \DIFdelbegin \DIFdel{$\xi_i \sim \mathcal{N}(0, \VarEnv)$ }\DIFdelend \DIFaddbegin \DIFadd{$\xi_{\Indiv} \sim \mathcal{N}(0, \VarEnv)$ }\DIFaddend is the effect of environment on the trait for individual \DIFdelbegin \DIFdel{i}\DIFdelend \DIFaddbegin \DIFadd{$\Indiv$}\DIFaddend .
    \item \DIFdelbegin \DIFdel{$\Trait_i = G_i + \xi_i$ }\DIFdelend \DIFaddbegin \DIFadd{$\Trait_{\Indiv} = G_{\Indiv}+ \xi_i$ }\DIFaddend is the phenotype for individual \DIFdelbegin \DIFdel{$i$}\DIFdelend \DIFaddbegin \DIFadd{$\Indiv$}\DIFaddend .
\end{itemize}
...
Here we \DIFdelbegin \DIFdel{generalize to }\DIFdelend \DIFaddbegin \DIFadd{model }\DIFaddend $\Ntrait$ traits evolving along the \DIFdelbegin \DIFdel{phylogeny and }\DIFdelend \DIFaddbegin \DIFadd{phylogenetic tree that }\DIFaddend are correlated between them.
...
The rate of change of the Brownian process is determined by the positive semi-definite and symmetric covariance matrix between traits $\CovarianceMatrix$ ($\Ntrait \times \Ntrait$).
Along branch \DIFdelbegin \DIFdel{$j$ with length $d_{j}$}\DIFdelend \DIFaddbegin \DIFadd{$\Branch$ with length $d_{\Branch}$}\DIFaddend , the Brownian process start at the ancestral node \DIFdelbegin \DIFdel{$\mathcal{A}(j)$ with value $\Brownian(\mathcal{A}(j))$}\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{A}(\Branch)$ with value $\Brownian(\mathcal{A}(\Branch))$}\DIFaddend , and ends at node \DIFdelbegin \DIFdel{$\mathcal{R}(j)$  with value $\Brownian(\mathcal{R}(j))$}\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{R}(\Branch)$  with value $\Brownian(\mathcal{R}(\Branch))$}\DIFaddend .
The independent contrast \DIFdelbegin \DIFdel{$\contrast_{j}$ }\DIFdelend \DIFaddbegin \DIFadd{$\contrast_{\Branch}$ }\DIFaddend defined as change in trait along the branch normalized by \DIFdelbegin \DIFdel{$\sqrt {d_{j}}$ }\DIFdelend \DIFaddbegin \DIFadd{$\sqrt {d_{\Branch}}$ }\DIFaddend is a multivariate Gaussian:
\begin{equation}
    \label{eq:DistribBrownian}
    \contrast\DIFdelbegin \DIFdel{_{j} }\DIFdelend \DIFaddbegin \DIFadd{_{\Branch} }\DIFaddend = \DIFdelbegin \DIFdel{\frac{\Brownian (\mathcal{R}(j)) - \Brownian (\mathcal{A}(j)) }{\sqrt {d_{j}}} }\DIFdelend \DIFaddbegin \DIFadd{\frac{\Brownian (\mathcal{R}(\Branch)) - \Brownian (\mathcal{A}(\Branch)) }{\sqrt {d_{\Branch}}} }\DIFaddend \sim \mathcal{N}\left(\VecZero, \CovarianceMatrix \right).
\end{equation}
...
\begin{equation}
    \Scattermatrix = \sum\limits\DIFdelbegin \DIFdel{_{j=1}}\DIFdelend \DIFaddbegin \DIFadd{_{\Branch=1}}\DIFaddend ^{\Nbranch} \contrast\DIFdelbegin \DIFdel{_{j} }\DIFdelend \DIFaddbegin \DIFadd{_{\Branch} }\DIFaddend \MultiplyMatrix \DIFdelbegin %DIFDELCMD < \left[%%%
\DIFdelend \contrast\DIFdelbegin \DIFdel{_{j}}%DIFDELCMD < \right]%%%
...
\begin{center}
    ...
    \includegraphics[width=1.0\textwidth, page=1] {\DIFadd{figureS2}}
