...
def open_trace_file(trace_file: str, burn_in: int) -> dict[str: np.ndarray]:
    ...
    for col in df.columns:
        if col.startswith("Var_"):
            ...
            out_dict[replace_last(col[4:], "_mean", "")] = df[col].values[burn_in:]
    n_taxa = int(df["n_taxa"][0])
    return out_dict, n_taxa
...
def main(inference_path: str, variance_pop_path: str, burn_in: int, output_path: str):
    ...
    traces_dict, n_taxa = open_trace_file(inference_path, burn_in=burn_in)
    ...
    for trait, trait_trace in traces_dict.items():
        ...
        ratio_pv = np.mean(trait_trace > variance_pop)
